...
import { ISSUE_CATEGORY_COLUMN_HEADER } from './issue-category-column.enum';
...
export class IssueCategoryComponent implements OnInit, OnDestroy {
  ...
  listen() {
    this.forEditListener = this.broadcast.on<string>('OPEN_ISSUE_CATEGORY_FOR_EDIT').subscribe((data: any) => {
      this.ngZone.run(() => {
        this.edit(null, data);
      });
    });

    this.forDeleteListener = this.broadcast.on<string>('OPEN_ISSUE_CATEGORY_FOR_DELETE').subscribe((data: any) => {
      this.ngZone.run(() => {
        this.delete(data);
      });
    });
  ...
  manipulate(res) {
    this.setResponse(res.issueCategoryMasterList);
    this.setColumnHeader(res.issueCategoryMasterList);
    this.setRowData(res.issueCategoryMasterList);
    this.activeListing.list = this.sampleData;
    this.sampleData.totalDocs = res.totalCount || res.issueCategoryMasterList.length;
  }

  setResponse(resData) {
    this.sampleData.currentPageNo = this.currentPageNo;
    this.sampleData.listingType = AppConstant.ISSUE_CATEGORY_LISTING_TYPE;
    ...
    this.sampleData.sortField = 'issueCatID';
  ...
  setColumnHeader(resData) {
    ...
    if (colData.length) {
      ...
      this.sampleData.columnHeader.push(ISSUE_CATEGORY_COLUMN_HEADER['srno']);
      for (let key in rowData) {
        if (ISSUE_CATEGORY_COLUMN_HEADER[key]) {
          this.sampleData.columnHeader.push(ISSUE_CATEGORY_COLUMN_HEADER[key]);
        }
      }
      this.sampleData.columnHeader.push(ISSUE_CATEGORY_COLUMN_HEADER['delete']);
    }
  }
  ...
  exportTableToExcel(type: string): void {
    ...
    /* save to file */
    XLSX.writeFile(wb, `issue-category-data.${type}`);
  ...
  delete(item?: any, i?: any) {
    ...
    if (r) {
      this.httpClient.post(ApiConstant.deleteIssueCategoryData + `?issuecatidlong=${item.issueCatID}`, null).subscribe((data) => {
        this.util.notification.success({
          title: 'Success',
          msg: 'Issue category details deleted successfully.'
        });
        this.loadData();
      }, (err) => {
        this.util.notification.error({
          title: 'Error',
          msg: 'Error while deleting issue category details!'
        })
      });
    }
  }
