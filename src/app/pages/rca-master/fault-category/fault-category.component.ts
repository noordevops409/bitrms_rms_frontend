...
import { FAULT_CATEGORY_COLUMN_HEADER } from './fault-category-column.enum';
...
@Component({
  selector: 'app-fault-category',
  templateUrl: './fault-category.component.html',
  styleUrls: ['./fault-category.component.scss']
})
export class FaultCategoryComponent implements OnInit, OnDestroy {
  ...
  listen() {
    this.forEditListener = this.broadcast.on<string>('OPEN_FAULT_CATEGORY_FOR_EDIT').subscribe((data: any) => {
      this.ngZone.run(() => {
        this.edit(null, data);
      });
    });

    this.forDeleteListener = this.broadcast.on<string>('OPEN_FAULT_CATEGORY_FOR_DELETE').subscribe((data: any) => {
      this.ngZone.run(() => {
        this.delete(data);
      });
    });
  ...
  manipulate(res) {
    this.setResponse(res.faultCategoryMasterList);
  ...
  setResponse(resData) {
    ...
    this.sampleData.listingType = AppConstant.FAULT_CATEGORY_LISTING_TYPE;
    ...
    this.sampleData.sortField = 'faultID';
  ...
  setColumnHeader(resData) {
    ...
    if (colData.length) {
      ...
      this.sampleData.columnHeader.push(FAULT_CATEGORY_COLUMN_HEADER['srno']);
      for (let key in rowData) {
        if (FAULT_CATEGORY_COLUMN_HEADER[key]) {
          this.sampleData.columnHeader.push(FAULT_CATEGORY_COLUMN_HEADER[key]);
        }
      }
      this.sampleData.columnHeader.push(FAULT_CATEGORY_COLUMN_HEADER['delete']);
    }
  }
  ...
  exportTableToExcel(type: string): void {
    ...
    /* save to file */
    XLSX.writeFile(wb, `fault-category-data.${type}`);
  ...
  add(evt?: any) {
    ...
    const dialogRef = this.dialog.open(AddFaultCategoryComponent, {
      width: '1000px',
      height: 'auto'
    });
  ...
  delete(item?: any, i?: any) {
    ...
    if (r) {
      this.httpClient.post(ApiConstant.deleteFaultCategoryData + `?faultidlong=${item.faultID}`, null).subscribe((data) => {
        this.util.notification.success({
          title: 'Success',
          msg: 'Fault category details deleted successfully.'
        });
        this.loadData();
      }, (err) => {
        this.util.notification.error({
          title: 'Error',
          msg: 'Error while deleting fault category details!'
        })
      });
    }
  }
