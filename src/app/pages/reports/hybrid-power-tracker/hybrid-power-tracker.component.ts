...
import { HYBRID_POWER_TRACKER_COLUMN_HEADER } from './hybrid-power-tracker-column.enum';
...
@Component({
  selector: 'app-hybrid-power-tracker',
  templateUrl: './hybrid-power-tracker.component.html',
  styleUrls: ['./hybrid-power-tracker.component.scss']
})
export class HybridPowerTrackerComponent implements OnInit {
  ...
  loadData() {
    ...
    this.isLoading = true;
    let apiUrl: any = ApiConstant.getHybridPowerTrackerReport + `/${this.currentPageNo}/size/${this.pageSize}`;
  ...
  setResponse(resData) {
    ...
    this.sampleData.listingType = AppConstant.HYBRID_POWER_TRACKER_LISTING_TYPE;
  ...
  setColumnHeader(resData) {
    ...
    if (colData.length) {
      ...
      for (let key in rowData) {
        if (HYBRID_POWER_TRACKER_COLUMN_HEADER[key]) {
          this.sampleData.columnHeader.push(HYBRID_POWER_TRACKER_COLUMN_HEADER[key]);
        }
      }
    }
  }
