...
import { HYBRID_ENERGY_RUN_HOURS_COLUMN_HEADER } from './hybrid-energy-hours-column.enum';
...
@Component({
  selector: 'app-hybrid-energy-run-hours',
  templateUrl: './hybrid-energy-run-hours.component.html',
  styleUrls: ['./hybrid-energy-run-hours.component.scss']
})
export class HybridEnergyRunHoursComponent implements OnInit {
  ...
  loadData() {
    ...
    this.isLoading = true;
    let apiUrl: any = ApiConstant.getHybridEnergyRunHoursReport + `/${this.currentPageNo}/size/${this.pageSize}`;
  ...
  setResponse(resData) {
    ...
    this.sampleData.listingType = AppConstant.HYBRID_ENERGY_RUN_HOURS_LISTING_TYPE;
  ...
  setColumnHeader(resData) {
    ...
    if (colData.length) {
      ...
      for (let key in rowData) {
        if (HYBRID_ENERGY_RUN_HOURS_COLUMN_HEADER[key]) {
          this.sampleData.columnHeader.push(HYBRID_ENERGY_RUN_HOURS_COLUMN_HEADER[key]);
        }
      }
    }
  }
