...
import { PLANNED_ENERGY_COLUMN_HEADER } from './planned-energy-column.enum';
...
export class PlannedEnergyComponent implements OnInit, OnDestroy {
  ...
  listen() {
    this.forEditListener = this.broadcast.on<string>('OPEN_PLANNED_ENERGY_FOR_EDIT').subscribe((data: any) => {
      this.ngZone.run(() => {
        this.edit(null, data);
      });
    });

    this.forDeleteListener = this.broadcast.on<string>('OPEN_PLANNED_ENERGY_FOR_DELETE').subscribe((data: any) => {
      this.ngZone.run(() => {
        this.delete(data);
      });
    });
  ...
  loadData() {
    ...
    this.isLoading = true;
    let apiUrl: any = ApiConstant.getPlannedEnergyMasterData;
  ...
  setColumnHeader(resData) {
    ...
    if (colData.length) {
      ...
      this.sampleData.columnHeader.push(PLANNED_ENERGY_COLUMN_HEADER['srno']);
      for (let key in rowData) {
        if (PLANNED_ENERGY_COLUMN_HEADER[key]) {
          this.sampleData.columnHeader.push(PLANNED_ENERGY_COLUMN_HEADER[key]);
        }
      }
      this.sampleData.columnHeader.push(PLANNED_ENERGY_COLUMN_HEADER['delete']);
    }
  }
  ...
  exportTableToExcel(type: string): void {
    ...
    /* save to file */
    XLSX.writeFile(wb, `planned-energy-data.${type}`);
  ...
  searchGlobally(event) {
    ...
    if (value) {
      this.sampleData.data = this.allData.data.filter((item) => {
        item.smSitecode = item.smSitecode.toString();
  ...
  add(evt?: any) {
    ...
    dialogRef.afterClosed().subscribe(data => {
      if (data) {
        if (data.plannedEnergyList && data.plannedEnergyList.length) {
  ...
  delete(item?: any, i?: any) {
    ...
    if (r) {
      this.httpClient.post(ApiConstant.deletePlannedEnergyMasterData + `?peSitecode=${item.smSitecode}`, null).subscribe((data) => {
        this.util.notification.success({
          title: 'Success',
          msg: 'Planned energy details deleted successfully.'
        });
        this.loadData();
      }, (err) => {
        this.util.notification.error({
          title: 'Error',
          msg: 'Error while deleting planned energy details!'
        })
      });
    }
  }
