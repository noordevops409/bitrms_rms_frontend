import { LITHIUM_COLUMNS_33_TO_48 } from '../lith-latestdata-33-48.enum';
...
@Component({
  selector: 'app-lith-latestdata-part3',
  templateUrl: './lith-latestdata-part3.component.html',
  styleUrls: ['./lith-latestdata-part3.component.scss']
})
export class LithLatestdataPart3Component implements OnInit, OnDestroy {
  ...
  loadData(): void {
    ...
    this.lithiumService.getLithiumPart3().subscribe(
      (res: any) => {
      ...
      (err) => {
        ...
        this.util.notification.error({ title: 'Error', msg: 'Failed to load Lithium Battery Data (Cells 33-48)' });
      }
    );
  }
  ...
  exportTableToExcel(type: string): void {
    ...
    XLSX.writeFile(wb, `lithium-battery-data-part3.${type}`);
  }
  ...
  setColumnHeader(res: any[]): void {
    ...
    if (res && res.length) {
      // Add base columns
      this.sampleData.columnHeader.push({ ...LITHIUM_COLUMNS_33_TO_48['smSitecode'], userIndex: 0 });
      this.sampleData.columnHeader.push({ ...LITHIUM_COLUMNS_33_TO_48['lastUpdated'], userIndex: 1 });
      this.sampleData.columnHeader.push({ ...LITHIUM_COLUMNS_33_TO_48['clusterName'], userIndex: 2 });
      this.sampleData.columnHeader.push({ ...LITHIUM_COLUMNS_33_TO_48['sitetypeid'], userIndex: 3 });
      this.sampleData.columnHeader.push({ ...LITHIUM_COLUMNS_33_TO_48['zone'], userIndex: 4 });
      this.sampleData.columnHeader.push({ ...LITHIUM_COLUMNS_33_TO_48['region'], userIndex: 5 });
      this.sampleData.columnHeader.push({ ...LITHIUM_COLUMNS_33_TO_48['powersource'], userIndex: 6 });
      this.sampleData.columnHeader.push({ ...LITHIUM_COLUMNS_33_TO_48['deviceType'], userIndex: 7 });

      // Add battery data columns 33-48
      Object.keys(res[0]).forEach((key) => {
        if (key.match(/^(liBattVoltage|liBattCurrent|liSOC|liSOH|liBattTemp)(3[3-9]|4[0-8])$/)) {
          if (LITHIUM_COLUMNS_33_TO_48[key]) {
            this.sampleData.columnHeader.push(LITHIUM_COLUMNS_33_TO_48[key]);
          }
        }
      });
    }
